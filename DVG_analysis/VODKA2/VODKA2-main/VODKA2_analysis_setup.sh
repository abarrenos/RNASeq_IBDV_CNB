...
\t \033[1m-k <VODKA_dir>\t\tDirectory containing VODKA scripts\033[0m (default: working directory)
...
###-- ADRIAN --### Set new option -k: Directory containing VODKA bash scripts

options=':h:f:d:v:k:p:g:r:n:'
...
###-- ADRIAN --### Save working directory (wd) where the bash scripts are contained
...
echo "cd $wd" > vodka_species_${project}_${DVGtype}.sh
...
while read i; do
	...
	cmd1="if [ ! -s ${alignment}/${samplename}.${vodkabt2_name}.sam ]; then bowtie2 -p 30 -x \"${vodkabt2}\" --local -U \"${i}\" -S ${alignment}/${samplename}.${vodkabt2_name}.sam --no-sq --no-unal --mp 0,0 ; fi "
	echo "${cmd1}" >> vodka_step1_${project}_${DVGtype}.sh

	# VODKA STEP 2: Extract reads matching DVG ref
	cmd2="perl $vodka_dir/search1.pl ${alignment}/${samplename}.${vodkabt2_name}.sam ${alignment}/${samplename}.${vodkabt2_name}.search_output.txt ${gl} ${rl}"
	...
	cmd3="perl $vodka_dir/organize.pl ${vodka_folder}/vodka_output_${DVGtype}/${samplename}_vodka_output ${vodkabt2_name} ${samplename}"
	echo "${cmd3}" >> vodka_step3_${project}_${DVGtype}.sh

###-- ADRIAN --### VODKA2_BLAST is contained in the working directory

	## BLAST VERIFICATION ##
	echo "cp ${results}/${vodka_suffix} ${samplename}_${vodka_suffix}" >> vodka_blast_${project}_${DVGtype}.sh
	echo "bash $vodka_dir/VODKA2_blast.sh -r ${virusfas_name} -v ${samplename}_${vodka_suffix} -s _${vodka_suffix} -t ${DVGtype}" >> vodka_blast_${project}_${DVGtype}.sh

	## EXTENDED REPORT WITH SPECIES ##
    if [ ${DVGtype} == "DEL" ]; then
      echo "bash $vodka_dir/VODKA2_report.sh -s ${samplename} -p ${project} -t ${DVGtype} -n ${number} -a *annotation.txt" >> vodka_species_${project}_${DVGtype}.sh
    else
	  echo "bash $vodka_dir/VODKA2_report.sh -s ${samplename} -p ${project} -t ${DVGtype} -n ${number}" >> vodka_species_${project}_${DVGtype}.sh
    fi
