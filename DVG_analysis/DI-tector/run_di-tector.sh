...
# Scripts
ditector=scripts/DI-tector_06.adrian.py
...
for i in $fastq_dir/*sample*_R1*; do
    ...
    (  if false; then
       if $paired ; then
           if [ ! -s ${sample}/${sample}_paired-end_counts.txt ]; then

               # Run DI-tector
	       echo -e "\nProcessing $group sample $sample Paired-End ..."
	       python3.8 $ditector $filter_host -o $sample\
	       -t "${sample}_paired-end" -d -k -p 0 -x $nproc \
	       $viral_ref $R1 -R2 $R2 2>&1 | tee log/${sample}_paired-end_di-tector.log

	       # Remove heavy temp files
	       rm ${sample}/${sample}_paired-end_temp_seqment.fq
	       rm ${sample}/${sample}_paired-end_temp_aln.sam
           fi      
       else 
  ...
  ## Convert SAM files to BAM files to reduce size
    ...
    ## Plot Coverage Depth of the Viral Genome and save png
       ...
       bash $plot_coverage $sample/${sample}_paired-end_temp_file_Virus_Coverage.txt \
       $sample/${sample}_paired-end_coverage_depth > log/${sample}_paired-end_plot_coverage.log 2>&1
    
    ## Parse the DI-tector output file and extract the names of reads catalogued as DVGs
    	
       cat $sample/${sample}_R1_output_sorted.txt | cut -f 1,9,10,17 > $sample/${sample}_R1_DVG_read_names.txt
	   cat $sample/${sample}_R1_output_sorted.txt | cut -f 1,9,10,17 > $sample/${sample}_R2_DVG_read_names.txt

	   # Combine both file
	   cat $sample/${sample}_R[12]_DVG_read_names.txt | sort -k1,4 | uniq > $sample/${sample}_R12_DVG_read_names.txt
...
ln -sf `realpath *sample*/*read_names.txt` summary/dvg_read_names
