...
if __name__ =='__main__':
	...
	parser.add_argument("R1", help="File containing single reads / forward paired-end reads (R1) in FASTQ format.")
	parser.add_argument("-R2", "--Paired_R2_reads", help="File containing reverse paired-end reads (R2) in FASTQ format (optional).")	
...
print ("Input file (R1):          {}".format(args.R1))
if args.Paired_R2_reads: #is not None:
	print ("Input file (R2):          {}".format(args.Paired_R2_reads))
...
## Reading Input File R1
if os.path.splitext(args.R1)[1]==".gz":
	print ("=================================\nReading gziped input file...\n=================================")
	open_file = gzip.open(args.R1, 'rb')			
else:
	print ("=================================\nReading input file...\n=================================")
	open_file = open(args.R1)
...
## Reading Input File R2
#if args.Paired_R2_reads:
#	if os.path.splitext(args.Paired_R2_reads)[1]==".gz":
#		print ("=================================\nReading gziped input file...\n=================================")
#		open_file = gzip.open(args.Paired_R2_reads, 'rb')			
#	else:
#		print ("=================================\nReading input file...\n=================================")
#		open_file = open(args.Paired_R2_reads)
#
#	no_line = 0
#	for line in open_file:
#		no_line+=1
...
#	open_file.close()
#	print ("=================================\n")
#	input_r2_nb_reads = no_line/factor_nb_reads


if args.Host_Ref:

### ADRIAN: Check if fastq file with Host-filtered reads exists. If so use backup file as input.

	if args.Paired_R2_reads:
		temp_file_woHost_r1 = out_dir+file_tag+'_temp_file_woHost_R1'+'.fq'
		temp_file_woHost_r2 = out_dir+file_tag+'_temp_file_woHost_R2'+'.fq'
	else:
	...
	#	with open(out_dir+file_tag+'_temp_file_alnHost'+'.bam', 'w') as out_file_woHost:
	#		p1 = Popen(["bwa", "mem", "-t", str(args.Nb_threads), args.Host_Ref, args.R1], stdout=PIPE)
	...
	if not os.path.exists(temp_file_woHost) or os.path.getsize(temp_file_woHost) == 0:
		with open(temp_file_woHost, 'w') as out_file_woHost:
			if args.Paired_R2_reads:
				p1 = Popen(["bwa", "mem", "-t", str(args.Nb_threads), args.Host_Ref, args.R1, args.Paired_R2_reads], stdout=PIPE)
			else:
				p1 = Popen(["bwa", "mem", "-t", str(args.Nb_threads), args.Host_Ref, args.R1], stdout=PIPE)
			p2 = Popen(["samtools", "view", "-S", "-f", "4", "-"], stdin=p1.stdout, stdout=PIPE)
			p1.stdout.close()
			if args.Paired_R2_reads:
				p3 = Popen(["samtools", "fastq", "-@", str(args.Nb_threads), "-1", temp_file_woHost_r1, "-2", temp_file_woHost_r2, "-"], stdin=p2.stdout, stdout=PIPE)
				p2.stdout.close()
				p3.communicate()[0]
			else:
				p3 = Popen(["samtools", "fastq", "-@", str(args.Nb_threads), "-"], stdin=p2.stdout, stdout=out_file_woHost) 
				p2.stdout.close()
				p3.communicate()[0]
                       
		print("=================================\nStep {}/{} : Done! ({:.0f}m {:.0f}s)\n=================================\n".format(no_step,total_step,int(time.time()-timer_start)/60,int(time.time()-timer_start)%60))
	else:
		print("=================================\nUsing already existing filtered fastq file without Host\n=================================\n")
else:
	if args.Paired_R2_reads:
		temp_file_woHost_r1 = args.R1
		temp_file_woHost_r2 = args.Paired_R2_reads
	else:
		temp_file_woHost = args.R1
...
if args.No_Quantification:
	...
	temp_file_woVirus=out_dir+file_tag+'_temp_file_woVirus'+'.sam'
	
	if not os.path.exists(temp_file_woVirus) or os.path.getsize(temp_file_Virus) == 0:
		with open(out_dir+file_tag+'_temp_file_woVirus'+'.sam', 'w') as out_file_woVirus:
			if args.Paired_R2_reads:
				p1 = Popen(["bwa", "mem", "-t", str(args.Nb_threads), args.Virus_Ref, temp_file_woHost_r1, temp_file_woHost_r2], stdout=PIPE)
			else:
...
### ADRIAN: Check if bam file with sequence mapping to the viral genome exists. If so use already existing file.
### Sequences in the bam file are filtered by FLAG = 4 (Non-mapped) and CIGAR ~ S (Soft) or H (Hard) clipping.
### Clipped reads are those that do not match completely to the reference sequence.
###
### Samtools view -F 4 only select reads that have been aligned to the reference and properly paired (both ends).

	temp_file_Virus=out_dir+file_tag+'_temp_file_Virus'+'.bam'
	
	if not os.path.exists(temp_file_Virus) or os.path.getsize(temp_file_Virus) == 0:

		# Mapping with bwa-mem, generates onVirus sam file
		with open(out_dir+file_tag+'_temp_file_onVirus'+'.sam', 'w') as out_file_onVirus:
			if args.Paired_R2_reads:
				Popen(["bwa", "mem", "-t", str(args.Nb_threads), args.Virus_Ref, temp_file_woHost_r1, temp_file_woHost_r2], stdout=out_file_onVirus).communicate()[0]
			else:
				Popen(["bwa", "mem", "-t", str(args.Nb_threads), args.Virus_Ref, temp_file_woHost], stdout=out_file_onVirus).communicate()[0]
		
		# Filtering unaligned reads into woVirus sam file
		...
		try:
			os.remove(out_dir+file_tag+'_temp_file_Virus_unsorted'+'.bam')
				...
				pass
### ADRIAN
### Use option -d 0 to deactivate maximum depth limit, otherwise max. depth = 8000 (Samtools 1.10).
		...
		with open(out_dir+file_tag+'_temp_file_Virus_Coverage'+'.txt', 'w') as out_file_Virus_Cov:
			Popen(["samtools", "depth", "-d 0", "-a", file_bam], stdout=out_file_Virus_Cov).communicate()[0]
...
### ADRIAN
### Even if we keep files, remove fragments and temporary alignments to reduce disk usage
