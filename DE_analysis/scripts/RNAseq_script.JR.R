...
# the reference genome data, which is in file Ggallus.gtf. This last file
...
if (use.online.annotation == TRUE) {
    cat("--------------------------------------------------------------\n")
    cat("    GETTING ENSEMBL ANNOTATION FROM THE NET\n")
    dir.create('net.EnsDb', showWarnings=FALSE)
    qr <- query(ah, c("EnsDb", target.organism))
    # choose the most recent (last) one
    last.ref <- names(qr)[length(names(qr))]
#    if ( ! file.exists(paste('net.EnsDb', '/net.ens.db.rds', sep='')) ) {
        cat("        Contacting ENSEMBL\n")
        # get ENSEMBL annotations for our target
        net.edb <- qr[[last.ref]]
        ens.db <- net.edb
        ...
        # and save as well as Rdata file
        save(ens.db, file=paste(paste('net.EnsDb', '/net.ens.db.RData', sep=''), sep=''))
        #	'ens.db' can later be recovered with: load(file='net.ens.db.RData')
#    } else {
...
if (FALSE) {
    ...
    if (use.online.annotation == TRUE) {
        cat("--------------------------------------------------------------\n")
        cat("    GETTING ORG.DB ANNOTATION FROM THE NET\n")
        cat("--------------------------------------------------------------\n")
    ...
    cat("--------------------------------------------------------------\n")
    cat("    LOADING ORG.DB ANNOTATION\n")
    cat("--------------------------------------------------------------\n")
    require(org.package, character.only=T)
    org.db <- eval(parse(text=org.package))
    columns(org.db)
    #org.db <- get(org.package)
}
...
if ( ! file.exists(paste(folder, 'biomaRt.annotation.1st.txt', sep='/')) ) {
    if ( ! file.exists(paste(folder, 'biomaRt.annotation.txt.gz', sep='/')) ) {
...
#	this will take the position counts and match them against the 
...
as.png( {
        plot(counts[,1:12], myCPM[,1:12], xlim=c(0,20), ylim=c(0,5))
        abline(v=10, col=1)
    }, out.png )
...
#thres <- myCPM > 0.25		# Coturnix
thres <- myCPM > 0.5		# Gallus
...
logcpm <- cpm(dge$counts, log=TRUE)
	### JR ###
# SID (smaple-id) contains a unique short identifier for each sample
#group.col <- c('red', 'blue', 'green', 'yellow')[target$Sample] 
ncol <- dim(target)[1]
group.col <- palette.colors(ncol, "Polychrome 36", recycle=T)     # ncol colors
	### JR ###
...
as.png( {
        ...
        heatmap.2(highly_var, 
                  col= rev(morecolors(ncol)), 
                  trace='none', 
                  ColSideColors=group.col[1:ncol(highly_var)],	### JR ###
                  scale='row', 
                  margins= c(15,5))
        #par(mar=margins)
    }, out.png)
...
n <- 50
high_var <- highly_var
colnames(high_var) <- gsub("_s.*", "", colnames(highly_var))
...
as.png( {
        #margins <- par("mar")
        #par(mar=c(25, 5, 5, 10))
        heatmap.2(high_var[1:n, ], 	### JR ###
                      col=rev(morecolors(50)), 
                      trace='none', 
                      ColSideColors=group.col[1:ncol(highly_var)],
                      scale='row', 
                      margins= c(15,5),
                      labRow=name[1:n, 2])
        #par(mar=margins)
...
#design.persist.inf
#design.persist.inf <- model.matrix(~ target$CellStatus * target$VirusStatus)
#design.persist.inf
...
eR.save.top(fit, folder, n.genes, 'p')
eR.save.top(fit, folder, n.genes, 'B')
eR.save.top(fit, folder, n.genes, 'logFC')
eR.save.top(fit, folder, n.genes, 'AveExpr')
...
# once we have prepared the database, we now want to extract some annotation 
...
eR.save.top.annotated(fit, folder, n.genes, sort.by='p')
eR.save.top.annotated(fit, folder, n.genes, sort.by='logFC')
eR.save.top.annotated(fit, folder, n.genes, sort.by='AveExpr')
...
summary(res.thres)
topTreat(fit.thres, coef=1, sort.by='p')
...
eR.save.top.ann.thresh(fit.thres, folder, n.genes, 'p')
eR.save.top.ann.thresh(fit.thres, folder, n.genes, 'logFC')
eR.save.top.ann.thresh(fit.thres, folder, n.genes, 'AveExpr')
...
fit.thres$genes <- merge(fit.thres$genes, bm.annot.1, by.x="GENEID", by.y="ensembl_gene_id")
head(fit.thres$genes)
...
eR.save.top.bm.ann.thresh(fit.thres, folder, n.genes, 'p')
eR.save.top.bm.ann.thresh(fit.thres, folder, n.genes, 'B')
eR.save.top.bm.ann.thresh(fit.thres, folder, n.genes, 'logFC')
eR.save.top.bm.ann.thresh(fit.thres, folder, n.genes, 'AveExpr')
...
#design.column <- 'viral.dose'	# Cj
#design.column <- 'Src'		# Gg
# for Gallus and Coturnix we will use 'PFU' after fixing SampleInfo.txt
design.column <- 'PFU'
design <- model.matrix(~0 + target[ , design.column])
colnames(design) <- levels(as.factor(target[ , design.column]))
...
if ( ! file.exists(paste(name, "rds", sep='.')) ) {
    ...
    dds <- DESeqDataSetFromMatrix(countData, colData,  design=~PFU, tidy=F)
