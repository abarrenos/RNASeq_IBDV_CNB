...
# XXX JR XXX
# createEnsDbForSpecies(ens_version = 104, user = "sci", host = "localhost", pass="1_4_All+All_4_1", local_tmp="mysql/coturnix_japonica_core_104_2", species="coturnix_japonica", dropDb=FALSE)
library(RMySQL)
...
createEnsDbForSpecies <- function(ftp_folder,
                                  ens_version = 104, species, user, host, pass,
...
#XXX JR XXX    res <- installEnsemblDb(dir = local_tmp, host = host, dbname = db_name,
#                            user = user, pass = pass, port = port)
    ## (3) Delete the downloads.
    fls <- dir(local_tmp, full.names = TRUE)
    #XXX JR XXX DON'T DELETE!!
    #res <- sapply(fls, unlink)
    
    ## (4) Create the EnsDb (requires the correct Ensembl API)
    ##     They are created in the local directory.
    # XXX JR XXX USE OUR OWN EXPLICIT VARIANT
    #cat("\nFetching tables from ENSEMBL\n")
    #fetchTablesFromEnsembl(version = "104", species = "coturnix_japonica", user="sci", host="localhost", pass="1_4_All+All_4_1")
# XXX JR XXX    fetchTablesFromEnsembl(version = ens_version, species = species,
#                           user = user, host = host, pass = pass, port = port)
    cat("\nMaking SQL from tables\n")
    # XXX JR XXX
    #DBFile <- makeEnsemblSQLiteFromTables()



    cat("\nMaking EnsDB package\n")
    ### JR ### Use an already existing sqlite file indtead
    gtffile <- "gtf/Gallus_gallus.GRCg6a.106.gtf"
    DB <- ensDbFromGtf(gtf=gtffile, verbose=TRUE)
    edb.gtf <- EnsDB(DB)    
    DBFile <- "gallus_gallus.GRCg6a.106.sqlite"
    
    makeEnsembldbPackage(DBFile, version="105", 
    		     maintainer="J. R. Valverde <jrvalverde@cnb.csic.es>", 
    		     author="J. R. Valverde",
                     destDir="./ensdb.gtf", license="Artistic-2.0")
    edb <- EnsDb(DBFile)
    
    # XXX JR XXX unlink("*.txt")
# XXX JR XXX
    
    ## (5) Delete the database.
    if (dropDb) {
        if (length(port))
            # XXX JR XXX MariaDB() -> MySQL()
...
downloadFilesFromFtpFolder <- function(url, dest = tempdir()) {
    ...
    cat("\nXXX JR XXX: processing", url, "returning", dest, "\n\n")
    message("OK")
    return(dest)	# XXX JR XXX
...
installEnsemblDb <- function(dir, host = "localhost", dbname, user, pass,
    ...
    tmp <- system(paste0("yes no | gunzip -k ", dir, "/*.gz"))	# XXX JR (yes no| .. -k) XXX
    ## Create the database
    if (length(port))
    	# XXX JR XXX MariaDB() -> MySQL()
        con <- dbConnect(MySQL(), host = host, user = user, pass = pass,
        ...
        con <- dbConnect(MySQL(), host = host, user = user, pass = pass,
                         dbname = "mysql")
    # XXX JR XXX Only create the database if it does not exist
    dbSendStatement(con, paste0("create database if not exists ", dbname))
    # XXX JR XXX dbDisconnect(con)
